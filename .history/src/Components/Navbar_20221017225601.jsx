...
import WallentConnect from './Wallet';
const Navbar = () => {
    const[nav,setNav]=useState(false);
    const handleNav=()=>{
        setNav(!nav);
    }
    const {WallentHandler}=useContext(WallentConnect);
  return (
           ...
           <div className='' ><img src={Logo}  className='w-6 h-5'/></div>
            <div className='hidden md:flex  '>
                <ul className='flex md:my-3 md:justify-evenly md:items-center md:flex-wrap text-white text-center'>
                    ...
                    <li  className='px-1 inline-flex items-center text-sm cursor-pointer hover:text-red-900' ><img src={metaLogo} className='w-4 h-4 mr-1'/> onClick={()=>{WallentHandler()}}  Add IP 2 MetaMask</li>
