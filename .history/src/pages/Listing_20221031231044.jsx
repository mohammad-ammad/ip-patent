...
function ImageParser(data)
{
  ...
  const ListingNFT = async (event) => {
    ...
    if (
      ...
      toast.promise(
        contract
          .list({value:0}, id, EtherPrice, formInput.amount)
          .then((transaction) => {
            toast.promise(
              transaction
                .wait()
          ...
          <div className="p-2  grid sm:grid-cols-1 justify-center md:grid-cols-3 lg:grid-cols-3 gap-1 ">
           {
            ...
            <div className="flex justify-between">
                <span className="inline-block text-white  rounded-full px-3 py-1 text-sm font-semibold  mr-10 mb-1">
                  Quantity 
                </span>
                <span className="inline-block text-white rounded-full px-3 py-1 text-sm font-semibold  mr-2 mb-1">
                  {item.amount} 
                </span>
