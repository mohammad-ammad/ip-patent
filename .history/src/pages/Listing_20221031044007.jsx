...
const MintingPage = () => {
  ...
  const ListingNFT = async (event) => {
    ...
    if(formInput.id !=='' ||formInput.amount !=='' || formInput.price !=='')
    {
...
//     const re= await contract.list(0,formInput.id, utils.parseEther( formInput.price),formInput.amount);
// console.log("reeee",re);
    toast.promise(
        contract.list(0,formInput.id,utils.parseEther( formInput.price),formInput.amount).then((transaction)=>{
            toast.promise(
                transaction
                .wait()
                .then((tx)=>{
