...
function ImageParser(data)
{
  ...
  const ListingNFT = async (event,id) => {
    ...
    )
    {
      ...
      let EtherPrice = utils.formatEther("100");
      console.log("Price in ethers:",EtherPrice);
      let wei=EtherPrice.toString(10);
      console.log("in weis:",wei)
      // ethers.utils.parseUnits("0.00000000000000001","ether")
      // BigNumber.from("42")
      toast.promise(
        contract
          .list( 0, 100,1,{value:ethers.BigNumber.from("0.00000000000000001")
        })
          .then((transaction) => {
            toast.promise(
              transaction
      ...
      // toast.promise(
      //   contract
      //     .list({value:ethers.utils.parseUnits("0","ether")}, id, wei, {value:1})
      //     .then((transaction) => {
      //       toast.promise(
      //         transaction
      //           .wait()
