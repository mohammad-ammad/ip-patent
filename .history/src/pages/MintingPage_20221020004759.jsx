...
const MintingPage = () => {
      ...
      try {
        ...
        toast.promise(
          axios({
            ...
            handleCreateMetadata("https://gateway.pinata.cloud/ipfs/"+responce?.data?.IpfsHash);
          }),
          {
  ...
  const mintItem = async (metadataURI) => {
    ...
    const deploy = contract.mint(formInput.quantity, metadataURI).then(tx => tx.wait( ).then( responce => console.log("Minted Responce:")));
    await deploy.wait();
