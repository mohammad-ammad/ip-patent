    ...
    if (formInput.name !== "") {
      try {
        ...
        toast.promise(axios({
          ...
          .then((responce) => {
            handleCreateMetadata(responce?.data?.IpfsHash);
          })
          .catch(error=>{"Upload File to Ipfs Error:", error}), {
            pending: "Uploading data to IPFS",
            error: "Something went wrong well upload PDF to IPFS",
            success:"Upload to IPFS"
          }
           );
